...
impl ConfidentialTransaction {
    ...
    pub fn new(
        ...
        recipient: [u8; 32],
        amount: u64,
        sender_keypair: &crate::keys::VibeKeypair,
    ) -> Self {
        ...
        let signature = crate::signer::sign_message(sender_keypair, &data);
    ...
    pub fn verify(&self, sender_pubkey: &crate::keys::VibePublicKey) -> bool {
        ...
        let signature = crate::signer::VibeSignature::try_from(&self.signature[..]).unwrap();
        
        match sender_pubkey.to_dalek_pubkey() {
            Ok(pubkey) => crate::signer::verify_signature(&data, &signature, &pubkey),
            Err(_) => false,
        }
    }
}
