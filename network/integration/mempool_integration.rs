...
impl MempoolIntegration {
    ...
    pub async fn handle_transaction(&self, tx: TransactionRecord, source_peer: &str) -> Result<(), MempoolError> {
    ...
    async fn broadcast_transaction(&self, tx: TransactionRecord, source_peer: &str) {
    ...
    pub async fn start_mempool_listener(&self) -> Result<(), String> {
        ...
        let mut rx = match &self.tx_subscription {
            Some(rx) => rx.clone(),
            ...
            while {
                ...
                match rx.recv().await {
                    Some(tx) => {
                        ...
                        match broadcaster.broadcast(NetMessage::NewTransaction(tx)).await {
                            Ok(_) => {
                                debug!("Transaction broadcast to peers");
                            },
                            Err(e) => {
