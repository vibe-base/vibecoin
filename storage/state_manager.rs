...
use crate::storage::state_store::{AccountState, AccountType, StateRoot, StateStoreError};
            ...
            Ok(Some(bytes)) => {
                match bincode::deserialize(&bytes) {
                    ...
                    Err(e) => {
                        ...
                        Err(StateStoreError::DeserializationError(e.to_string()))
                    }
                }
            },
            Ok(None) => Ok(None),
            ...
            if let Some(addr) = Schema::extract_account_address(&key_str) {
                ...
                match bincode::deserialize(&value) {
                    ...
                    Err(e) => {
                        ...
                        return Err(StateStoreError::DeserializationError(e.to_string()));
                    }
                }
            }
        }
    ...
    pub fn apply_block(&self, block: &Block) -> Result<(), StateStoreError> {
        ...
        for (_, tx_hash_bytes) in tx_entries {
            ...
            let new_sender_state = AccountState {
                ...
                account_type: sender_state.account_type,
            };
            ...
            let new_recipient_state = AccountState {
                ...
                account_type: recipient_state.account_type,
            };
